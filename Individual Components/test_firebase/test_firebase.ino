...
void loop(){
  if (Firebase.ready() && signupOK && (millis() - sendDataPrevMillis > 15000 || sendDataPrevMillis == 0)){
    ...
    if (Firebase.RTDB.setInt(&fbdo, "test/Temperature", count)){
    ...
    if (Firebase.RTDB.setFloat(&fbdo, "test/Humidity", 0.01 + random(0,100))){
      Serial.println("PASSED");
      Serial.println("PATH: " + fbdo.dataPath());
      Serial.println("TYPE: " + fbdo.dataType());
    }
    ...
    if (Firebase.RTDB.setString(&fbdo, "test/Time", "Random Time")){
      Serial.println("PASSED");
      Serial.println("PATH: " + fbdo.dataPath());
      Serial.println("TYPE: " + fbdo.dataType());
    }
